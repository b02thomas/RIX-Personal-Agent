...
from app.models.n8n import (
    N8NStatusResponse, N8NWorkflowInfo, N8NExecutionInfo,
    WorkflowTriggerRequest, WorkflowTriggerResponse, ExecutionStatus,
    WorkflowDiscoveryRequest, WorkflowActivationRequest, WorkflowActivationResponse,
    AITriggeredExecutionRequest, WorkflowAnalyticsResponse, WorkflowSyncRequest,
    WorkflowSyncResponse, WorkflowPerformanceMetrics
)
...
async def get_n8n_status(current_user: AuthenticatedUser = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get N8N status: {str(e)}"
        )
...
async def get_execution(
    ...
    try:
        ...
        logger.error(
            "Failed to get execution info",
            execution_id=execution_id,
            error=str(e)
        )
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get execution info: {str(e)}"
        )
...
async def discover_workflows(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Workflow discovery failed: {str(e)}"
        )
...
async def activate_workflow(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Workflow activation failed: {str(e)}"
        )
...
async def trigger_ai_workflow(
    ...
    try:
        ...
        logger.error(
            "AI-triggered workflow execution failed",
            workflow_type=request.workflow_type.value,
            error=str(e)
        )
        raise HTTPException(
            status_code=500,
            detail=f"AI-triggered workflow execution failed: {str(e)}"
        )
...
async def sync_workflows(
    request: WorkflowSyncRequest,
    current_user: AuthenticatedUser = Depends(get_current_user)
):
    """Sync workflows from N8N instance"""
    logger.info(
        "Workflow sync requested",
        user_id=current_user.user_id,
...
async def _track_ai_triggered_execution(workflow_type: str, execution_time: float, 
    ...
    try:
        ...
        if workflow:
            ...
            logger.info(
                "AI-triggered execution tracked",
                workflow_id=workflow['id'],
                workflow_type=workflow_type,
                success=success
            )
        ...
        logger.error(
            "Failed to track AI-triggered execution",
            workflow_type=workflow_type,
            error=str(e)
        )
...
async def _execute_workflow_background(n8n_request, user_id: str):
    ...
    try:
        ...
        if response.execution_id:
            ...
            await _track_ai_triggered_execution(
                response.workflow_type.value,
                response.processing_time,
                True,
                user_id
