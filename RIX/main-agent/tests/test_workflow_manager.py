...
class TestWorkflowManager:
    ...
    async def test_discover_and_sync_workflows_success(self, manager, mock_workflows):
        ...
        with patch("app.services.database.database") as mock_db, patch("app.services.n8n_client.n8n_client") as mock_n8n:
    ...
    async def test_concurrent_sync_prevention(self, manager):
        ...
        async def slow_sync():
            ...
            return WorkflowSyncResponse(
                synced_count=1, updated_count=0, new_count=1, categories=["test"], success=True, message="Success"
            )
