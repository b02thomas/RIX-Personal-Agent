...
from app.models.schemas import IntelligenceResponse, ProjectHealthResponse
...
class ProjectHealthService:
    ...
    async def assess_project_health(self, user_id: str, project_id: str) -> ProjectHealthResponse:
        ...
        try:
            ...
            if not project:
                return ProjectHealthResponse(success=False, data={}, health_score=0, insights={"error": "Project not found"})

            # Get project tasks
            tasks = await database.execute(
                "SELECT * FROM tasks WHERE project_id = $1 ORDER BY created_at ASC", uuid.UUID(project_id), fetch=True
            )
            ...
            optimization_suggestions = self._generate_optimization_suggestions(project, tasks, risk_factors, health_score)

            # Predict completion
            completion_prediction = self._predict_completion(project, tasks, task_velocity_health)

            # Store health metrics
            await self._store_health_metrics(
                project_id, health_score, risk_factors, success_indicators, optimization_suggestions, completion_prediction
            )
                    ...
                    "key_metrics": {
                        ...
                        "overdue_tasks": len(
                            [
                                t
                                for t in tasks
                                if t.get("due_date") and t["due_date"] < datetime.now() and t["status"] != "completed"
                            ]
                        ),
            ...
            return ProjectHealthResponse(success=False, data={}, health_score=0, insights={"error": str(e)})
    ...
    def _assess_timeline_health(self, project: Dict[str, Any], tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        overdue_tasks = [
            t for t in tasks if t.get("due_date") and t["due_date"].date() < date.today() and t["status"] != "completed"
        ]
        ...
        for dep in dependencies:
            if dep["task_status"] != "completed" and dep["dependent_task_status"] in ["todo", "in_progress"]:
                blocked_tasks.append(
                    {"task": dep["dependent_task_title"], "blocked_by": dep["task_title"], "type": dep["dependency_type"]}
                )
    ...
    def _calculate_overall_health_score(self, completion: Dict, timeline: Dict, velocity: Dict, dependencies: Dict) -> int:
    ...
    def _identify_risk_factors(
        self, project: Dict, tasks: List[Dict], completion: Dict, timeline: Dict, velocity: Dict
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_optimization_suggestions(
        self, project: Dict, tasks: List[Dict], risk_factors: List[Dict], health_score: int
    ) -> List[Dict[str, Any]]:
    ...
    def _predict_completion(self, project: Dict, tasks: List[Dict], velocity: Dict) -> Dict[str, Any]:
        """Predict project completion date and confidence"""
        incomplete_tasks = len([t for t in tasks if t["status"] != "completed"])

        if incomplete_tasks == 0:
            return {"status": "completed", "estimated_date": None, "confidence": 1.0, "days_ahead_behind": 0}

        weekly_velocity = velocity.get("weekly_velocity", 0.5)

        if weekly_velocity <= 0:
            return {"status": "stalled", "estimated_date": None, "confidence": 0.1, "days_ahead_behind": None}
    ...
    async def _store_health_metrics(
        ...
        await database.execute(
            ...
            json.dumps(completion_prediction),
        )
    ...
    def _calculate_velocity(self, completed_tasks: List[Dict], days: int) -> float:
        ...
        recent_completions = [t for t in completed_tasks if t["completion_date"] >= cutoff_date]
