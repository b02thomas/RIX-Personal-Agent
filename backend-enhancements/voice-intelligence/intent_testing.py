...
class IntentTestingSuite:
    ...
    def __init__(self):
        ...
        self.logger.info("Intent Testing Suite initialized with {} test cases".format(
            len(self.test_cases)
        ))
    ...
    async def _run_single_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            # Classify intent
            actual_result = await self.intent_classifier.classify_intent(
                test_case.text, 
                test_case.context
            )
            ...
            entity_match = await self._validate_entities(
                actual_result.entities, 
            ...
            return TestResult(
                ...
                error_message="" if passed else f"Expected: {test_case.expected_intent.value}, Got: {actual_result.intent.value}"
            )
    ...
    async def _calculate_performance_metrics(
        ...
        return {
            ...
            "overall_performance": "EXCELLENT" if all(targets_met.values()) else "GOOD" if accuracy >= 0.8 else "NEEDS_IMPROVEMENT"
        }
    ...
    async def _generate_test_report(self) -> Dict[str, Any]:
        ...
        return {
            "test_summary": {
                ...
                "test_completion_rate": len([r for r in self.test_results if r.actual_result is not None]) / len(self.test_results)
            },
    ...
    async def _generate_recommendations(self) -> List[str]:
        ...
        if accuracy < 0.8:
            recommendations.append("Critical: Accuracy is below 80%. Review intent patterns and add more specific German language patterns.")
        
        # Confidence recommendations
        if avg_confidence < 0.7:
            recommendations.append(f"Average confidence is {avg_confidence:.1%}. Improve pattern specificity and confidence scoring.")
        
        # Entity extraction recommendations
        if entity_rate < 0.7:
            recommendations.append(f"Entity extraction rate is {entity_rate:.1%}. Enhance German language entity extraction patterns.")
        
        # Performance recommendations
        if processing_time > 0.5:
            recommendations.append(f"Processing time is {processing_time:.3f}s. Optimize pattern matching for better performance.")
    ...
    async def run_performance_benchmark(self, iterations: int = 100) -> Dict[str, Any]:
        ...
        benchmark_cases = [
            ...
            if test.expected_intent != IntentCategory.GENERAL_CONVERSATION
        ][:10]  # Top 10 diverse cases
    ...
    async def test_cultural_context_awareness(self) -> Dict[str, Any]:
        ...
        for test_pair in cultural_test_cases:
            ...
            intent_consistency = (
                formal_result.intent == informal_result.intent == test_pair["expected_intent"]
            )
