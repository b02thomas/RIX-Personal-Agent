    ...
    def _generate_tasks(self) -> List[Dict[str, Any]]:
        ...
        for i, (title, priority, status) in enumerate(task_templates):
            task = {
                ...
                "tags": random.sample(["frontend", "backend", "design", "testing", "documentation", "bug", "feature"], k=random.randint(1, 3)),
...
class EnhancedMockResponses:
    ...
    def _generate_task_response(
        ...
        if action == "create_task":
            ...
            response = template["response"].format(
                ...
                priority=priority,
                due_date=due_date or "no specific date"
            )
            ...
            task_list = "\n".join([
                f"• {task['title']} ({task['priority']} priority) - {task['status']}"
                for task in tasks
            ])
    ...
    def generate_voice_response(self, audio_data: str, format: str = "webm", context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "command": {
                "action": intent,
                "parameters": self._extract_voice_parameters(transcription, intent)
            },
            "metadata": {
                "format": format,
