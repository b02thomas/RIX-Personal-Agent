...
from app.models.schemas import (  # Project models; Task models; Response models
    APIResponse,
    PaginatedResponse,
    PaginationParams,
    Project,
...
async def create_project(project_data: ProjectCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        project = await core_api_service.create_project(user_id=current_user["user_id"], project_data=project_data)
        return project
...
async def get_projects(
    ...
    try:
        ...
        projects = await core_api_service.get_projects(user_id=current_user["user_id"], pagination=pagination)
        return projects
    except Exception as e:
...
async def update_project(
    project_id: str = Path(...), update_data: ProjectUpdate = ..., current_user: dict = Depends(get_current_user)
):
    """Update project"""
    try:
        project = await core_api_service.update_project(
            user_id=current_user["user_id"], project_id=project_id, update_data=update_data
        )
...
async def delete_project(project_id: str = Path(...), current_user: dict = Depends(get_current_user)):
    """Delete project"""
    try:
        success = await core_api_service.delete_project(user_id=current_user["user_id"], project_id=project_id)
...
async def get_project_health(project_id: str = Path(...), current_user: dict = Depends(get_current_user)):
...
async def create_task(task_data: TaskCreate, current_user: dict = Depends(get_current_user)):
    """Create a new task"""
    try:
        task = await core_api_service.create_task(user_id=current_user["user_id"], task_data=task_data)
        return task
    except Exception as e:
...
async def get_tasks(
    ...
    try:
        ...
        tasks = await core_api_service.get_tasks(
            user_id=current_user["user_id"], pagination=pagination, project_id=project_id, status=status
        )
        return tasks
    except Exception as e:
...
async def get_task(task_id: str = Path(...), current_user: dict = Depends(get_current_user)):
...
async def update_task(task_id: str = Path(...), update_data: TaskUpdate = ..., current_user: dict = Depends(get_current_user)):
    """Update task"""
    try:
        ...
        where_conditions = {"user_id": uuid.UUID(current_user["user_id"]), "id": uuid.UUID(task_id)}

        query, params = SecureQueryBuilder.build_update_query(
            table_name="tasks", update_fields=update_fields, where_conditions=where_conditions, add_updated_at=True
        )
...
async def delete_task(task_id: str = Path(...), current_user: dict = Depends(get_current_user)):
    ...
    try:
        result = await core_api_service.database.execute(
            "DELETE FROM tasks WHERE user_id = $1 AND id = $2", uuid.UUID(current_user["user_id"]), uuid.UUID(task_id)
        )
...
async def complete_task(task_id: str = Path(...), current_user: dict = Depends(get_current_user)):
...
async def bulk_update_tasks(task_ids: List[str], update_data: TaskUpdate, current_user: dict = Depends(get_current_user)):
        ...
        for task_id in task_ids:
            try:
                task = await core_api_service.update_task(
                    user_id=current_user["user_id"], task_id=task_id, update_data=update_data
                )
                if task:
                    updated_count += 1
            except Exception as e:
...
async def get_task_analytics_summary(days: int = Query(30, ge=1, le=365), current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "avg_completion_time_hours": float(stats["avg_completion_time_minutes"]) / 60
                if stats["avg_completion_time_minutes"]
                else 0,
...
async def get_project_health_trends(days: int = Query(30, ge=7, le=90), current_user: dict = Depends(get_current_user)):
    ...
    try:
        health_history = await core_api_service.database.execute(
            ...
            WHERE p.user_id = $1 
            AND phm.calculated_at >= NOW() - INTERVAL $2 * INTERVAL '1 day'
