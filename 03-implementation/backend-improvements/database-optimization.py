...
class OptimizedDatabaseService:
    ...
    async def optimize_database(self) -> Dict[str, Any]:
        ...
        try:
            async with self.get_connection() as conn:
                ...
                tables = [
                    "conversations", "messages", "users", "projects", 
                    "tasks", "n8n_executions", "n8n_workflows"
                ]
    ...
    async def store_voice_interaction(self, user_id: str, audio_data: Dict[str, Any], transcription: str, intent_result: Dict[str, Any]) -> str:
        ...
        try:
            async with self.connection_pool.acquire() as conn:
                ...
                patterns = {
                    ...
                    "average_confidence": sum(row["avg_confidence"] * row["usage_count"] for row in results) / max(sum(row["usage_count"] for row in results), 1),
            ...
            async with self.connection_pool.acquire() as conn:
                ...
                for index_sql in mobile_indexes:
                    try:
                        ...
                        optimization_results["mobile_optimizations"].append(f"Created mobile index: {index_sql.split()[-1] if 'idx_' in index_sql else 'unknown'}")
                    except Exception as e:
