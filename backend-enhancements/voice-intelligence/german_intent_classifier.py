...
class MultiLanguageIntentClassifier:
    ...
    def __init__(self, language_utils: Optional[GermanLanguageUtils] = None):
        ...
        self.language_detection = {
            "german_keywords": ["ich", "du", "sie", "der", "die", "das", "und", "oder", "mit", "für", "auf", "von", "zu", "in", "an", "bei", "nach", "vor", "über", "unter", "durch", "gegen", "ohne", "um"],
            "english_keywords": ["i", "you", "the", "a", "an", "and", "or", "but", "with", "for", "on", "from", "to", "in", "at", "by", "after", "before", "about", "under", "through", "against", "without", "around"],
        ...
        total_patterns = sum(len(patterns) for patterns in self.intent_patterns.values()) + sum(len(patterns) for patterns in self.english_patterns.values())
        self.logger.info("Multi-Language Intent Classifier initialized with {} patterns (German: {}, English: {})".format(
            total_patterns,
    ...
    async def detect_language(self, text: str) -> Tuple[str, float]:
        ...
        german_indicators = sum(1 for indicator in ["ä", "ö", "ü", "ß", "sie", "ich", "der", "die", "das"] if indicator in text_lower)
    ...
    async def classify_intent(
        ...
        try:
            ...
            self.logger.debug(f"Classifying intent for text: '{text}' -> '{normalized_text}' (detected: {detected_language}, confidence: {language_confidence:.2f})")
            ...
            enhanced_entities = await self._extract_entities(
                normalized_text, 
                intent, 
                entities, 
                context,
        ...
        for intent_category, pattern_list in pattern_set.items():
            for pattern in pattern_list:
                ...
                if match:
                    ...
                    entities = await self._extract_pattern_entities(
                        match, 
                        pattern, 
                        text
                    )
    ...
    async def _calculate_confidence(
        ...
        final_confidence = min(1.0, max(0.0, 
            base_confidence + coverage_bonus + context_bonus + german_bonus - length_penalty
        ))
    ...
    async def get_classification_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                self.classification_stats["successful_classifications"] / total
                if total > 0 else 0.0
            ),
            "high_confidence_rate": (
                self.classification_stats["high_confidence_classifications"] / total
                if total > 0 else 0.0
            ),
            ...
            "target_accuracy_met": (
                self.classification_stats["successful_classifications"] / total >= 0.9
