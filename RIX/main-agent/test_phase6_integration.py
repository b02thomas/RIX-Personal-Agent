...
async def test_database_n8n_workflows_table():
    ...
    try:
        ...
        tracking_result = await database.track_workflow_execution(
            workflow_id="test-workflow-001", execution_time=2.5, success=True, ai_triggered=True
        )
        ...
        print(
            f"✅ Analytics retrieved: {summary.get('total_workflows', 0)} workflows, {summary.get('total_executions', 0)} executions"
        )
...
async def test_workflow_manager():
    ...
    try:
        ...
        print("\\nTesting workflow retrieval by category...")
        productivity_workflows = await workflow_manager.get_workflows_by_category(category="productivity", active_only=True)
    ...
    try:
        ...
        for message in test_messages:
            message_insights = await ai_workflow_intelligence.process_message_for_triggers(
                user_id=test_user_id, message=message, conversation_id="test-conv-789"
            )
