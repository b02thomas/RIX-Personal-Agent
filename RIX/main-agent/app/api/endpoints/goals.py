...
from app.models.schemas import (  # Goal models; Response models
    APIResponse,
    Goal,
    GoalCreate,
    GoalProgress,
...
async def create_goal(goal_data: GoalCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        goal = await core_api_service.create_goal(user_id=current_user["user_id"], goal_data=goal_data)
        return goal
...
async def get_goals(
    ...
    try:
        goals = await core_api_service.get_goals(user_id=current_user["user_id"], status=status)
...
async def get_goal(goal_id: str = Path(...), current_user: dict = Depends(get_current_user)):
...
async def update_goal(goal_id: str = Path(...), update_data: GoalUpdate = ..., current_user: dict = Depends(get_current_user)):
    """Update goal"""
    try:
        # Prepare update fields with proper JSON handling
        update_fields = {}
        ...
        where_conditions = {"user_id": uuid.UUID(current_user["user_id"]), "id": uuid.UUID(goal_id)}

        query, params = SecureQueryBuilder.build_update_query(
            table_name="user_goals", update_fields=update_fields, where_conditions=where_conditions, add_updated_at=True
        )
...
async def delete_goal(goal_id: str = Path(...), current_user: dict = Depends(get_current_user)):
    """Delete goal"""
    try:
        result = await core_api_service.database.execute(
            "DELETE FROM user_goals WHERE user_id = $1 AND id = $2", uuid.UUID(current_user["user_id"]), uuid.UUID(goal_id)
        )
...
async def record_goal_progress(
    progress_data: GoalProgressCreate, goal_id: str = Path(...), current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        progress = await core_api_service.record_goal_progress(user_id=current_user["user_id"], progress_data=progress_data)
        return progress
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid goal ID format")
    except Exception as e:
...
async def get_goal_progress(
    ...
    try:
        ...
        progress_entries = await core_api_service.database.execute(query, *params, fetch=True)
        ...
        if progress_entries:
            ...
            avg_confidence = (
                sum(p.get("confidence_level", 0) for p in progress_entries if p.get("confidence_level")) / total_entries
            )
        ...
        return {
            ...
            "progress_entries": [
                {**dict(entry), "recorded_at": entry["recorded_at"].isoformat()} for entry in progress_entries
            ],
        }
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid goal ID format")
...
async def get_goal_milestones(goal_id: str = Path(...), current_user: dict = Depends(get_current_user)):
...
async def create_goal_milestone(
    milestone_data: dict, goal_id: str = Path(...), current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        milestone = await core_api_service.database.execute(
            ...
            milestone_data.get("target_value"),
            milestone_data.get("target_date"),
            milestone_data.get("reward"),
            fetch_one=True,
        )

        return {**dict(milestone), "created_at": milestone["created_at"].isoformat()}
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid goal ID format")
    except Exception as e:
...
async def get_goal_analytics_summary(current_user: dict = Depends(get_current_user)):
...
async def get_goal_progress_trends(
    days: int = Query(90, ge=7, le=365), goal_type: Optional[str] = Query(None), current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        progress_data = await core_api_service.database.execute(query, *params, fetch=True)
        ...
        daily_averages = {date_key: sum(values) / len(values) for date_key, values in daily_progress.items()}

        confidence_averages = {date_key: sum(values) / len(values) for date_key, values in confidence_trends.items()}
        ...
        if len(daily_averages) >= 2:
            ...
            trend_direction = (
                "improving" if last_week_avg > first_week_avg else "declining" if last_week_avg < first_week_avg else "stable"
            )
        else:
        ...
        return {
            ...
            "trend_analysis": {
                ...
                "most_active_day": max(daily_progress.items(), key=lambda x: len(x[1]))[0].isoformat()
                if daily_progress
                else None,
            },
            "daily_trends": [
...
async def get_goal_specific_analytics(
    goal_id: str = Path(...), days: int = Query(60, ge=7, le=365), current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        if total_entries == 0:
            return {
                ...
                "analytics": {"total_entries": 0, "message": "No progress entries found for the specified period"},
            }
        ...
        avg_confidence = (
            sum(e.get("confidence_level", 0) for e in progress_history if e.get("confidence_level")) / total_entries
        )
        avg_energy = sum(e.get("energy_level", 0) for e in progress_history if e.get("energy_level")) / total_entries

        # Milestone analysis
        milestones = await core_api_service.database.execute(
            "SELECT * FROM goal_milestones WHERE goal_id = $1 ORDER BY target_value", uuid.UUID(goal_id), fetch=True
        )
        ...
        return {
            ...
            "analytics": {
                ...
                "completion_percentage": (goal["current_value"] / goal["target_value"] * 100)
