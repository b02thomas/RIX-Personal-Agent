...
from app.models.schemas import (  # Routine models; Response models
    APIResponse,
    Routine,
...
async def create_routine(routine_data: RoutineCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        routine = await core_api_service.create_routine(user_id=current_user["user_id"], routine_data=routine_data)
        return routine
...
async def get_routines(
    active_only: bool = Query(True), routine_type: Optional[str] = Query(None), current_user: dict = Depends(get_current_user)
):
    """Get user routines"""
    try:
        routines = await core_api_service.get_routines(user_id=current_user["user_id"], active_only=active_only)
...
async def get_routine(routine_id: str = Path(...), current_user: dict = Depends(get_current_user)):
...
async def update_routine(
    routine_id: str = Path(...), update_data: RoutineUpdate = ..., current_user: dict = Depends(get_current_user)
):
    """Update routine"""
    try:
        ...
        where_conditions = {"user_id": uuid.UUID(current_user["user_id"]), "id": uuid.UUID(routine_id)}

        query, params = SecureQueryBuilder.build_update_query(
            table_name="user_routines", update_fields=update_fields, where_conditions=where_conditions, add_updated_at=True
        )
...
async def delete_routine(routine_id: str = Path(...), current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return APIResponse(success=True, message="Routine deactivated successfully")
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid routine ID format")
    except Exception as e:
...
async def record_routine_completion(
    completion_data: RoutineCompletionCreate, routine_id: str = Path(...), current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        completion = await core_api_service.record_routine_completion(
            user_id=current_user["user_id"], completion_data=completion_data
        )
        return completion
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid routine ID format")
...
async def get_routine_completions(
    ...
    try:
        ...
        completions = await core_api_service.database.execute(query, *params, fetch=True)

        return {
            "routine_id": routine_id,
            "total_completions": len(completions),
            "completions": [
                {**dict(completion), "habits_completed": completion.get("habits_completed", [])} for completion in completions
            ],
        }
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid routine ID format")
...
async def get_routine_streak(routine_id: str = Path(...), current_user: dict = Depends(get_current_user)):
    """Get current streak for routine"""
    try:
        streak_info = await core_api_service.get_routine_streak(user_id=current_user["user_id"], routine_id=routine_id)
        ...
        return {
            ...
            "streak_quality": "excellent"
            if streak_info["current_streak"] >= 21
            else "good"
...
async def get_routine_coaching(routine_id: str = Path(...), current_user: dict = Depends(get_current_user)):
    ...
    try:
        coaching_insights = await routine_coaching_service.get_coaching_insights(
            user_id=current_user["user_id"], routine_id=routine_id
        )
...
async def get_completion_coaching(
    completion_data: dict, routine_id: str = Path(...), current_user: dict = Depends(get_current_user)
):
    """Get immediate coaching feedback after routine completion"""
    try:
        coaching_insights = await routine_coaching_service.get_coaching_insights(
            user_id=current_user["user_id"], routine_id=routine_id, recent_completion=completion_data
        )
        return coaching_insights
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid routine ID format")
...
async def get_routine_analytics_summary(days: int = Query(30, ge=1, le=365), current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "avg_completion_percentage": float(routine_stats["avg_completion_percentage"])
                if routine_stats["avg_completion_percentage"]
                else 0,
...
async def get_routine_patterns(days: int = Query(60, ge=14, le=365), current_user: dict = Depends(get_current_user)):
...
async def get_routine_specific_analytics(
    routine_id: str = Path(...), days: int = Query(30, ge=7, le=365), current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        avg_quality = (
            sum(c.get("quality_rating", 0) for c in completions if c.get("quality_rating"))
            / len([c for c in completions if c.get("quality_rating")])
            if completions
            else 0
        )
        ...
        for habit in all_habits:
            ...
            completed_count = sum(1 for c in completions if habit_id in c.get("habits_completed", []))
...
async def bulk_update_routine_completion(
    date_to_update: date, routine_completions: List[dict], current_user: dict = Depends(get_current_user)
):
