...
class ContextManager:
    ...
    async def _fetch_user_projects(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(
                    f"{self.rix_frontend_url}/api/projects", headers={"X-Internal-Request": "main-agent"}
                )
                response.raise_for_status()
                data = response.json()
    ...
    async def _fetch_time_blocks(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(
                    f"{self.rix_frontend_url}/api/calendar/time-blocks", headers={"X-Internal-Request": "main-agent"}
                )
                response.raise_for_status()
                data = response.json()
