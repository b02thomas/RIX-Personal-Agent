// /src/app/api/main-agent/n8n/sync/route.ts
// N8N workflow synchronization API proxy to main agent
// Forwards workflow sync requests to main agent for N8N instance synchronization
...
// POST /api/main-agent/n8n/sync - Sync workflows from N8N instance
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${mainAgentUrl}/api/n8n/sync`, {
      method: 'POST',
      headers: {
    ...
    console.error('N8N workflow sync error:', {
      error: error instanceof Error ? error.message : 'Unknown error',
      stack: error instanceof Error ? error.stack : undefined,
      endpoint: '/api/main-agent/n8n/sync',
      timestamp: new Date().toISOString()
    });
    return NextResponse.json(
      { 
