...
from app.models.n8n import (
    N8NWorkflowInfo, WorkflowPerformanceMetrics, WorkflowCategoryInfo,
    WorkflowSyncResponse, WorkflowActivationResponse
)
...
class WorkflowManager:
    ...
    def __init__(self):
        ...
        self.categorization_rules = {
            "productivity": [
                "task", "calendar", "schedule", "project", "deadline",
                "todo", "planning", "organize", "management"
            ],
            "communication": [
                "chat", "message", "email", "notification", "alert",
                "conversation", "voice", "speech", "talk"
            ],
            "intelligence": [
                "news", "brief", "analysis", "insight", "learn",
                "routine", "coaching", "optimization", "smart", "ai"
            ],
            "automation": [
                "automation", "trigger", "webhook", "integration",
                "sync", "connect", "api", "workflow"
            ],
            "analytics": [
                "analytics", "metrics", "report", "dashboard",
                "data", "statistics", "performance", "tracking"
            ]
        }
        ...
        try:
            ...
            for workflow in discovered_workflows:
                try:
                    ...
                    logger.error(
                        "Failed to sync individual workflow",
                        workflow_id=workflow.id,
                        error=str(e)
                    )
    ...
    async def get_workflows_by_category(self, category: str = None, 
