...
class TestAIWorkflowIntelligence:
    ...
    def mock_user_activity(self):
        ...
        return {
            "conversations": [
                ...
                {
                    "id": "conv-003",
                    "title": "Schedule optimization help",
    ...
    def test_initialization(self, intelligence):
        ...
        expected_patterns = ["morning_routine", "calendar_heavy", "project_status", 
                           "routine_optimization", "task_overwhelm"]
    ...
    async def test_analyze_user_patterns_comprehensive(self, intelligence, mock_user_activity):
        ...
        with patch.object(intelligence, '_get_recent_user_activity', return_value=mock_user_activity), \
             patch('app.services.ai_workflow_intelligence.datetime') as mock_datetime:
    ...
    async def test_analyze_user_patterns_low_confidence_filtering(self, intelligence):
        ...
        with patch.object(intelligence, '_get_recent_user_activity', return_value=low_activity), \
             patch('app.services.ai_workflow_intelligence.datetime') as mock_datetime:
    ...
    async def test_schedule_intelligence_analysis(self, intelligence):
        ...
        with patch.object(intelligence, 'analyze_user_patterns', return_value=high_priority_insights), \
             patch.object(intelligence, 'execute_intelligence_insights') as mock_execute:
    ...
    async def test_schedule_intelligence_analysis_no_high_priority(self, intelligence):
        ...
        with patch.object(intelligence, 'analyze_user_patterns', return_value=low_priority_insights), \
             patch.object(intelligence, 'execute_intelligence_insights') as mock_execute:
...
class TestAIWorkflowIntelligenceIntegration:
    ...
    async def test_rix_prd_compliance_no_llm_calls(self):
        ...
        for message in test_messages:
            insights = await intelligence.process_message_for_triggers(
                "user-test", message, "conv-test"
            )
    ...
    async def test_pattern_based_intelligence_comprehensive(self):
        ...
        for scenario in test_scenarios:
            insights = await intelligence.process_message_for_triggers(
                "user-test", scenario["message"], "conv-test"
            )
            
            # Should find at least one insight matching the expected workflow
            matching_insights = [
                i for i in insights 
                if i.trigger_workflow == scenario["expected_workflow"]
            ]
