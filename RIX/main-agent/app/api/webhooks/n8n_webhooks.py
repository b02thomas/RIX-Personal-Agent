...
from app.models.n8n import N8NWebhookData, ExecutionStatus
...
async def handle_successful_execution(webhook_data: N8NWebhookData, raw_data: dict):
    ...
    try:
        ...
        if isinstance(webhook_data.result, dict):
            response_text = (
                webhook_data.result.get("response") or 
                webhook_data.result.get("message") or 
                webhook_data.result.get("output") or
                str(webhook_data.result)
            )
        ...
        if webhook_data.conversation_id and response_text:
            try:
                ...
                logger.error(
                    "Failed to create AI message from webhook",
                    execution_id=webhook_data.execution_id,
                    error=str(e)
                )
        ...
        logger.error(
            "Failed to handle successful execution",
            execution_id=webhook_data.execution_id,
            error=str(e)
        )
...
async def handle_failed_execution(webhook_data: N8NWebhookData):
    ...
    try:
        ...
        logger.error(
            "Failed to handle failed execution",
            execution_id=webhook_data.execution_id,
            error=str(e)
        )
