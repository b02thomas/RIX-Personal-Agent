...
from app.models.chat import (
    ...
    AIProcessingRequest, AIProcessingResponse, WorkflowType
)
    ...
    try:
        ...
        if websocket_manager.is_user_connected(current_user.user_id):
            background_tasks.add_task(
                websocket_manager.send_message_update,
        ...
        content_analysis = await message_router.analyze_content(
            request.content,
        ...
        if websocket_manager.is_user_connected(current_user.user_id):
            background_tasks.add_task(
                websocket_manager.send_ai_response,
                request.conversation_id,
                ai_message,
                current_user.user_id
            )
        ...
        background_tasks.add_task(
            _process_ai_intelligence_triggers,
            current_user.user_id,
            request.content,
            request.conversation_id
        )
...
async def analyze_user_intelligence(
    current_user: AuthenticatedUser = Depends(get_current_user)
):
...
async def schedule_intelligence_analysis(
    ...
    try:
        ...
        background_tasks.add_task(
            ai_workflow_intelligence.schedule_intelligence_analysis,
            current_user.user_id
        )
...
async def _process_ai_intelligence_triggers(user_id: str, message: str, conversation_id: str):
    ...
    try:
        logger.info(
            "Processing AI intelligence triggers",
            user_id=user_id,
            conversation_id=conversation_id
        )
        ...
        if insights:
            logger.info(
                "AI intelligence insights generated",
                user_id=user_id,
                insights_count=len(insights)
            )
            ...
            high_confidence_insights = [
                ...
                if insight.confidence >= 0.85  # High threshold for auto-execution
            ]
        ...
        logger.error(
            "Failed to process AI intelligence triggers",
            user_id=user_id,
            error=str(e)
        )