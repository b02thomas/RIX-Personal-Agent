...
class ErrorCode(str, Enum):
    ...
    # SECURITY NOTE: These are error code constants, not passwords or secrets
    # They are used for consistent error identification across the application
...
class ErrorResponse:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.details:
            result["details"] = [
                {"field": detail.field, "value": detail.value, "message": detail.message, "location": detail.location}
...
class DatabaseError(RIXException):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            details=details,
            status_code=503,
            suggestions=["Please try again in a few moments", "If the problem persists, contact support"],
        )
...
class ResourceNotFoundError(RIXException):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            details=details,
            status_code=404,
            suggestions=["Verify the resource ID is correct", "Check if the resource exists and you have access to it"],
        )
...
class ErrorHandler:
    ...
    def handle_exception(
        self, exception: Exception, request: Optional[Request] = None, user_id: Optional[str] = None
    ) -> ErrorResponse:
    ...
    def _handle_rix_exception(
        self, exception: RIXException, request_id: str, user_id: Optional[str], endpoint: Optional[str]
    ) -> ErrorResponse:
        """Handle RIX-specific exceptions"""
        ...
        return ErrorResponse(
            ...
            support_info={"request_id": request_id, "contact": "support@rix.com"},
        )

    def _handle_validation_error(
        self, exception: RequestValidationError, request_id: str, user_id: Optional[str], endpoint: Optional[str]
    ) -> ErrorResponse:
        """Handle FastAPI validation errors"""

        logger.warning(
            "Validation error", errors=exception.errors(), request_id=request_id, user_id=user_id, endpoint=endpoint
        )
        ...
        return ErrorResponse(
            ...
            support_info={"request_id": request_id, "documentation": "https://docs.rix.com/api"},
        )
    ...
    def _handle_http_exception(
        ...
        return ErrorResponse(
            ...
            support_info={"request_id": request_id, "status_code": exception.status_code},
        )

    def _handle_generic_exception(
        self, exception: Exception, request_id: str, user_id: Optional[str], endpoint: Optional[str]
    ) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            user_id=user_id,
            endpoint=endpoint,
            suggestions=["Please try again in a few moments", "If the problem persists, contact support with the request ID"],
            support_info={"request_id": request_id, "contact": "support@rix.com", "timestamp": datetime.utcnow().isoformat()},
        )
    ...
    def _get_suggestions_for_status_code(self, status_code: int) -> List[str]:
        ...
        suggestions = {
            ...
            401: ["Check your authentication credentials", "Ensure your token is valid and not expired"],
            403: ["Verify you have permission to access this resource", "Contact your administrator if needed"],
            404: ["Check the resource ID or URL", "Verify the resource exists and is accessible"],
            429: ["Please wait before making another request", "Consider reducing request frequency"],
            500: ["Please try again in a few moments", "Contact support if the problem persists"],
    ...
    def _create_user_friendly_message(self, exception: Exception, error_code: ErrorCode) -> str:
        ...
        return (
            mobile_voice_messages.get(exception_type)
            or user_friendly_messages.get(exception_type)
            or "An unexpected error occurred while processing your request"
        )

    def create_error_response(self, error_response: ErrorResponse) -> JSONResponse:
        """Create FastAPI JSON response from error response"""

        status_code = self.status_code_mappings.get(error_response.error_code, 500)
...
def create_validation_error(field: str, value: Any, message: str, location: str = "body") -> ValidationError:
    """Create a validation error for a specific field"""
    detail = ErrorDetail(field=field, value=value, message=message, location=location)

    return ValidationError(
        error_code=ErrorCode.INVALID_FIELD_FORMAT, message=f"Validation failed for field '{field}'", details=[detail]
    )
...
def create_not_found_error(resource_type: str, resource_id: str) -> ResourceNotFoundError:
    ...
    return ResourceNotFoundError(
        ...
        details=[ErrorDetail(field="id", value=resource_id, message=f"No {resource_type} found with this ID")],
    )
...
def create_database_error(operation: str, table: Optional[str] = None) -> DatabaseError:
    ...
    return DatabaseError(error_code=ErrorCode.DATABASE_CONNECTION_ERROR, message=message)
...
def create_workflow_error(workflow_type: str, reason: str) -> WorkflowError:
    ...
    return WorkflowError(
        ...
        details=[ErrorDetail(field="workflow_type", value=workflow_type, message=reason)],
    )


def create_voice_processing_error(reason: str, audio_format: Optional[str] = None) -> RixError:
    """Create a voice processing error"""
    details = [ErrorDetail(field="processing", value="voice_input", message=reason)]

    if audio_format:
        details.append(
            ErrorDetail(
                field="audio_format", value=audio_format, message=f"Audio format '{audio_format}' may not be supported"
            )
        )

    return RixError(
        error_code=ErrorCode.VOICE_PROCESSING_FAILED, message=f"Voice processing failed: {reason}", details=details
    )
...
def create_mobile_optimization_error(device_info: Dict[str, Any], reason: str) -> RixError:
    ...
    return RixError(
        ...
        details=[
            ErrorDetail(
                field="device_type", value=device_info.get("type", "unknown"), message=f"Device compatibility issue: {reason}"
            ),
            ErrorDetail(
                field="user_agent",
...
def create_intelligence_error(intelligence_type: str, reason: str) -> RixError:
    ...
    return RixError(
        ...
        details=[ErrorDetail(field="intelligence_type", value=intelligence_type, message=reason)],
    )
...
class MobileErrorHandler:
    ...
    def handle_touch_error(touch_event: Dict[str, Any], error: Exception) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            details=[ErrorDetail(field="touch_type", value=touch_event.get("type", "unknown"), message=str(error))],
            suggestions=["Try tapping more gently", "Check if the screen is clean", "Try using a different finger"],
        )
    ...
    def handle_gesture_error(gesture_type: str, error: Exception) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            details=[ErrorDetail(field="gesture_type", value=gesture_type, message=str(error))],
            suggestions=[
    ...
    def handle_haptic_error(haptic_pattern: str, error: Exception) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            details=[ErrorDetail(field="haptic_pattern", value=haptic_pattern, message=str(error))],
            suggestions=[
...
class VoiceErrorHandler:
    ...
    def handle_speech_recognition_error(error: Exception, audio_info: Dict[str, Any]) -> ErrorResponse:
        ...
        if "network" in error_message:
            ...
            suggestions = ["Speak more clearly", "Reduce background noise", "Try speaking closer to microphone"]
        ...
        return ErrorResponse(
            ...
            details=[
                ErrorDetail(field="audio_format", value=audio_info.get("format", "unknown"), message=str(error)),
                ErrorDetail(field="duration", value=str(audio_info.get("duration", 0)), message="Audio duration in seconds"),
            ],
    ...
    def handle_audio_processing_error(error: Exception, audio_data: Dict[str, Any]) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            details=[
                ErrorDetail(field="error_type", value=type(error).__name__, message=str(error)),
                ErrorDetail(field="audio_size", value=str(audio_data.get("size", 0)), message="Audio file size in bytes"),
            ],
            suggestions=["Try recording a shorter message", "Check microphone quality", "Ensure stable internet connection"],
        )
...
class IntelligenceErrorHandler:
    ...
    def handle_routine_coaching_error(error: Exception, routine_data: Dict[str, Any]) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            details=[ErrorDetail(field="routine_count", value=str(routine_data.get("count", 0)), message=str(error))],
            suggestions=["Ensure you have routine data", "Try again in a few minutes", "Check your routine tracking history"],
        )
    ...
    def handle_project_intelligence_error(error: Exception, project_id: str) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            details=[ErrorDetail(field="project_id", value=project_id, message=str(error))],
            suggestions=[
    ...
    def handle_calendar_optimization_error(error: Exception, calendar_data: Dict[str, Any]) -> ErrorResponse:
        ...
        return ErrorResponse(
            ...
            details=[ErrorDetail(field="events_count", value=str(calendar_data.get("events", 0)), message=str(error))],
            suggestions=[
