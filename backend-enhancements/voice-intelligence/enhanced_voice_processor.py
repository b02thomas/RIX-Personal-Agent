...
from multilang_intent_classifier import IntentCategory, MultiLanguageIntentClassifier
from voice_mcp_router import MCPResponse, VoiceMCPRouter
...
class EnhancedVoiceProcessor:
    ...
    def __init__(self, openai_api_key: str, n8n_base_url: str, n8n_api_key: str = None, target_language: str = "auto"):
        ...
        self.whisper_configs = {
            ...
            "auto": {"model": "whisper-1", "temperature": 0.15, "response_format": "verbose_json"},
        }
        ...
        self.logger.info(
            f"Enhanced Multi-Language Voice Processor initialized (target: {target_language}, N8N: {n8n_base_url})"
        )

    async def process_voice_input(
        self, user_id: str, audio_data: bytes, session_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            transcription_result = await self._transcribe_audio(processed_audio, processing_id, metadata)

            # Post-process transcription for German language
            enhanced_result = await self._enhance_german_transcription(transcription_result, processing_id)
            ...
            if intent_result:
                ...
                extracted_entities = await self.entity_extractor.extract_entities(enhanced_result["text"], detected_language)
    ...
    async def _transcribe_audio(
        self, audio_data: bytes, processing_id: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                with open(temp_file_path, "rb") as audio_file:
                    response = await asyncio.to_thread(
                        self.openai_client.audio.transcriptions.create, file=audio_file, **whisper_params
                    )
                ...
                if hasattr(response, "segments") and response.segments:
                    confidences = [segment.get("avg_logprob", 0.0) for segment in response.segments]
    ...
    async def _enhance_german_transcription(self, transcription_result: Dict[str, Any], processing_id: str) -> Dict[str, Any]:
        """
    ...
    async def _classify_intent(
        self, text: str, processing_id: str, context: Optional[Dict[str, Any]] = None
        ...
        try:
            self.logger.debug(
                ...
                extra={"processing_id": processing_id, "text_length": len(text), "has_context": context is not None},
            )
            ...
            self.logger.error(
                ...
                extra={"processing_id": processing_id, "error": str(e), "text_length": len(text)},
            )
            return None
    ...
    async def process_audio_chunks(self, user_id: str, audio_chunks: List[bytes], session_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return await self.process_voice_input(
                user_id=user_id, audio_data=combined_audio, session_id=session_id, metadata={"chunk_count": len(audio_chunks)}
            )
    ...
    async def get_processing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (self.processing_stats["successful_requests"] / total_requests if total_requests > 0 else 0.0),
            ...
            "intent_classification_rate": (intent_classifications / total_requests if total_requests > 0 else 0.0),
            "high_confidence_rate": (
                self.processing_stats["high_confidence_intents"] / intent_classifications
                if intent_classifications > 0
                else 0.0
            ),
            ...
            "entity_extraction_rate": (
                self.processing_stats["entity_extractions"] / total_requests if total_requests > 0 else 0.0
            ),
            # MCP routing metrics
            "mcp_requests": self.processing_stats["mcp_requests"],
            "mcp_routing_rate": (self.processing_stats["mcp_requests"] / total_requests if total_requests > 0 else 0.0),
            ...
            "voice_response_rate": (
                ...
                if self.processing_stats["mcp_requests"] > 0
                else 0.0
            ),
            # End-to-end performance
            "average_end_to_end_time": (
                sum(self.processing_stats["end_to_end_times"]) / len(self.processing_stats["end_to_end_times"])
                if self.processing_stats["end_to_end_times"]
                else 0.0
            ),
            "end_to_end_target_met": (
                sum(1 for t in self.processing_stats["end_to_end_times"] if t < 3.0)
                / len(self.processing_stats["end_to_end_times"])
                if self.processing_stats["end_to_end_times"]
                else 0.0
            )
            >= 0.9,
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_response = await asyncio.to_thread(self.openai_client.models.list)
            openai_healthy = True
        except Exception as e:
