...
// POST /api/n8n/webhook/calendar-intelligence - Receive calendar intelligence updates
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Insert calendar intelligence message
      const calendarMessageResult = await client.query(
      ...
      const calendarMessage = calendarMessageResult.rows[0];
      ...
      // socket.emit('calendar-intelligence', { conversationId, message: calendarMessage, metadata });
    ...
    console.error('Calendar intelligence webhook error:', error);
