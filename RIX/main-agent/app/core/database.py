...
class DatabaseManager:
    ...
    async def execute(
        self, query: str, *args, fetch: bool = False, fetch_one: bool = False, fetch_val: bool = False
    ...
    async def vector_search(
        ...
        if not SecureQueryBuilder.validate_table_name(table):
            raise ValueError(f"Invalid table name: {table}")

        # Validate embedding column name (should be 'embedding' for our schema)
        if embedding_column != "embedding":
            raise ValueError(f"Invalid embedding column name: {embedding_column}")

        # Build query with validated identifiers
        safe_table = SecureQueryBuilder.sanitize_identifier(table)
        safe_embedding_column = SecureQueryBuilder.sanitize_identifier(embedding_column)

        query = f"""
            SELECT *, 
                   1 - ({safe_embedding_column} <=> %s::vector) as similarity_score
            FROM {safe_table}
        """
    ...
    async def upsert(
        ...
        for col in columns + conflict_columns + update_columns:
            if not SecureQueryBuilder.validate_field_name(table, col):
                raise ValueError(f"Invalid field name for table {table}: {col}")

        # Build column lists and placeholders with sanitized identifiers
        safe_table = SecureQueryBuilder.sanitize_identifier(table)
        safe_columns = [SecureQueryBuilder.sanitize_identifier(col) for col in columns]
        placeholders = [f"${i+1}" for i in range(len(columns))]
        values = list(data.values())

        # Build conflict resolution with sanitized identifiers
        safe_conflict_cols = [SecureQueryBuilder.sanitize_identifier(col) for col in conflict_columns]
        conflict_cols = ", ".join(safe_conflict_cols)

        safe_update_columns = [SecureQueryBuilder.sanitize_identifier(col) for col in update_columns]
        update_sets = [f"{col} = EXCLUDED.{col}" for col in safe_update_columns]
    ...
    async def bulk_insert(self, table: str, records: List[Dict[str, Any]], batch_size: int = 1000) -> int:
        ...
        for i in range(0, len(records), batch_size):
            ...
            # Build VALUES clause with sanitized identifiers
            safe_table = SecureQueryBuilder.sanitize_identifier(table)
            safe_columns = [SecureQueryBuilder.sanitize_identifier(col) for col in columns]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            vector_test = await self.execute("SELECT '[1,2,3]'::vector <=> '[1,2,3]'::vector as similarity", fetch_val=True)
...
async def get_user_by_id(user_id: str) -> Optional[Dict[str, Any]]:
    """Get user by ID"""
    return await database.execute("SELECT * FROM users WHERE id = $1 AND is_active = true", uuid.UUID(user_id), fetch_one=True)


async def get_user_by_email(email: str) -> Optional[Dict[str, Any]]:
    """Get user by email"""
    return await database.execute("SELECT * FROM users WHERE email = $1 AND is_active = true", email, fetch_one=True)


async def create_user(user_data: Dict[str, Any]) -> Dict[str, Any]:
    """Create new user"""
    return await database.upsert("users", user_data, ["email"], ["hashed_password", "full_name", "updated_at"])
...
async def get_user_dashboard_summary(user_id: str) -> Dict[str, Any]:
    ...
    return await database.execute(
        "SELECT * FROM v_user_dashboard_summary WHERE user_id = $1", uuid.UUID(user_id), fetch_one=True
    )
