...
from app.models.schemas import (  # Project & Task Models; Calendar Models; Routine Models; Knowledge Models; Goal Models; Analytics Models; Pagination
    BehavioralAnalytics,
    BehavioralAnalyticsCreate,
    CalendarEvent,
    CalendarEventCreate,
    CalendarEventUpdate,
    Goal,
...
class CoreAPIService:
    ...
    async def create_project(self, user_id: str, project_data: ProjectCreate) -> Project:
        ...
        try:
            ...
            await self.log_interaction(user_id, "task", "create_project", project_data.dict(), project.dict(), processing_time)

            return project

        except Exception as e:
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "task", "create_project", project_data.dict(), None, processing_time, str(e))
            raise
    ...
    async def update_project(self, user_id: str, project_id: str, update_data: ProjectUpdate) -> Optional[Project]:
        ...
        where_conditions = {"user_id": uuid.UUID(user_id), "id": uuid.UUID(project_id)}

        query, params = SecureQueryBuilder.build_update_query(
            table_name="projects", update_fields=update_fields, where_conditions=where_conditions, add_updated_at=True
        )
    ...
    async def delete_project(self, user_id: str, project_id: str) -> bool:
        """Delete project"""
        result = await database.execute(
            "DELETE FROM projects WHERE user_id = $1 AND id = $2", uuid.UUID(user_id), uuid.UUID(project_id)
        )
    ...
    async def create_task(self, user_id: str, task_data: TaskCreate) -> Task:
        ...
        try:
            ...
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "task", "create_task", task_data.dict(), task.dict(), processing_time)

            return task

        except Exception as e:
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "task", "create_task", task_data.dict(), None, processing_time, str(e))
            raise

    async def get_tasks(
        self, user_id: str, pagination: PaginationParams, project_id: Optional[str] = None, status: Optional[str] = None
    ) -> PaginatedResponse:
        """Get paginated tasks for user with optional filters"""
        # Build secure query with filters using SecureQueryBuilder
        where_conditions = {"user_id": uuid.UUID(user_id)}

        if project_id:
            where_conditions["project_id"] = uuid.UUID(project_id)

        if status:
            where_conditions["status"] = status

        # Build WHERE clause using SecureQueryBuilder
        where_clause, params = SecureQueryBuilder.build_where_conditions("tasks", where_conditions)
    ...
    async def create_calendar_event(self, user_id: str, event_data: CalendarEventCreate) -> CalendarEvent:
        ...
        try:
            ...
            result = await database.execute(
                ...
                data["is_all_day"],
                data["is_recurring"],
                json.dumps(data.get("recurrence_rule")),
                data["priority"],
                data.get("productivity_category"),
                data["tags"],
                json.dumps(data["metadata"]),
                fetch_one=True,
            )
            ...
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "calendar", "create_event", event_data.dict(), event.dict(), processing_time)

            return event

        except Exception as e:
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "calendar", "create_event", event_data.dict(), None, processing_time, str(e))
            raise
    ...
    async def detect_calendar_conflicts(
        self, user_id: str, start_time: datetime, end_time: datetime, exclude_event_id: Optional[str] = None
    ) -> List[CalendarEvent]:
        ...
        if exclude_event_id:
            query = """
                SELECT * FROM calendar_events 
                WHERE user_id = $1 AND start_time < $3 AND end_time > $2 AND id != $4
                ORDER BY start_time
            """
            params = [uuid.UUID(user_id), start_time, end_time, uuid.UUID(exclude_event_id)]
        else:
            query = """
                SELECT * FROM calendar_events 
    ...
    async def create_routine(self, user_id: str, routine_data: RoutineCreate) -> Routine:
        ...
        try:
            ...
            await self.log_interaction(
                user_id, "routine", "create_routine", routine_data.dict(), routine.dict(), processing_time
            )
            ...
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(
                user_id, "routine", "create_routine", routine_data.dict(), None, processing_time, str(e)
            )
            raise
    ...
    async def get_routine_streak(self, user_id: str, routine_id: str) -> Dict[str, Any]:
        ...
        current_streak = await database.execute(
            "SELECT calculate_routine_streak($1, $2) as streak", uuid.UUID(user_id), uuid.UUID(routine_id), fetch_val=True
        )
        ...
        return {"current_streak": current_streak or 0, "longest_streak": longest_streak or 0}
    ...
    async def create_knowledge_entry(self, user_id: str, entry_data: KnowledgeEntryCreate) -> KnowledgeEntry:
        ...
        try:
            ...
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "knowledge", "create_entry", entry_data.dict(), entry.dict(), processing_time)

            return entry

        except Exception as e:
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "knowledge", "create_entry", entry_data.dict(), None, processing_time, str(e))
            raise
    ...
    async def search_knowledge_entries(self, user_id: str, search_request: KnowledgeSearchRequest) -> List[Dict[str, Any]]:
        ...
        return [{"entry": KnowledgeEntry(**row), "similarity_score": float(row.get("relevance_score", 0))} for row in result]
    ...
    async def create_goal(self, user_id: str, goal_data: GoalCreate) -> Goal:
        ...
        try:
            ...
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "goal", "create_goal", goal_data.dict(), goal.dict(), processing_time)

            return goal

        except Exception as e:
            processing_time = int((datetime.now() - start_time).total_seconds() * 1000)
            await self.log_interaction(user_id, "goal", "create_goal", goal_data.dict(), None, processing_time, str(e))
            raise
    ...
    async def record_goal_progress(self, user_id: str, progress_data: GoalProgressCreate) -> GoalProgress:
        ...
        result = await database.execute(
            ...
            data.get("recorded_at") or datetime.now(),
            fetch_one=True,
        )
    ...
    async def get_behavioral_analytics(
        self, user_id: str, analysis_type: Optional[str] = None, period: Optional[str] = None
    ) -> List[BehavioralAnalytics]:
